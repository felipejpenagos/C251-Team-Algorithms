...
def exchange_analysis(design_var, constants):
    ...
    for day in range(1, days - 1):
        ...
        # --- EDIT #1: quality metric (MODIFIED - removed volatility term) ---
        quality[d, :] = (
        ...
        # --- EDIT #2: trading decisions (MODIFIED - removed buyable check) ---
        ...
        stocks_to_sell = np.array([
            s for s in owned_idx
            if quality[d, s] < sell_threshold
        ], dtype=int)

        #WE DO NOT WANT THIS! FORCING TO KEEP BAD STOCKS!!!
        ...
        # --- EDIT #3: execute buys (MODIFIED - top 3 stocks only) ---
        shares_to_buy = np.zeros(stocks)
        candidates = potentially_buyable

        # Switching to aggressive investment style and taking only top 3 stocks instead of all buyable
        if len(candidates) > 3:
            idx = np.argsort(quality[d, candidates])[-3:]
            stocks_to_buy = candidates[idx]
        else:
            stocks_to_buy = candidates
        ...
        # --- EDIT #4: threshold update (unchanged) ---
        ...
        # --- EDIT #5: minimum threshold gap (MODIFIED - 0.20 instead of 0.10) ---
        sell_threshold = min(
            buy_threshold - 0.20 * abs(buy_threshold), 
            sell_threshold
        )
...
if __name__ == "__main__":
    # Reading 200 day data from CSV
    url = "https://people.duke.edu/~hpgavin/cee251/stock_prices1.csv"
    df = pd.read_csv(url)
    df.to_csv("stock_prices1.csv", index=False)
    ...
    constants = [1, stock_prices]
    cost, _ = exchange_analysis(design_var, constants)

    # Print final result
    print("Final portfolio value: $", -cost)